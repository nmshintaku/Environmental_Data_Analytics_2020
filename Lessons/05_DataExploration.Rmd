...
```{r, fig.height = 3, fig.width = 4}
#histogram of discharge mean 
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

#histogram of discharge mean with bins counting by 10s
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#histogram of discharge mean with a number of 20 bins
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

#histogram of discharge with bin width of 10 and adding a limit to the x-axis values
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
#histogram of gage height mean
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#freq line plot for gage height mean, min, max; lty is line type
ggplot(USGS.flow.data) +
...
summary(USGS.flow.data$gage.height.max) #gives NA number 

#freq line plot of gage height mean based off approval or provisional
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#boxplot of gage height mean vs approved or provisional
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#boxplots of gage height mean and discharge mean
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#fancy violin plot with lines for each quartile
ggplot(USGS.flow.data) +
  geom_violin(aes(x = gage.height.mean.approval, y = gage.height.mean), 
              draw_quantiles = c(0.25, 0.5, 0.75)) #can use scale = count; default is scale = area
```
