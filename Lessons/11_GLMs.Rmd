...
# Test for normality. greater than .05 --> normal distribution
...
summary(Litter.Totals.anova) #plotID is significant predictor of dry mass 
...
# Post-hoc test: are any of the plot IDs significantly diff from other plot Ids
TukeyHSD(Litter.Totals.anova)

# Extract groupings for pairwise relationships, plot ID is predictor variable
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE)
Litter.Totals.groups
#groups that match are not significantly different
...
#stat_summary: what type of geom (text) and then assign labels in exact order in the dataset
```

What information does the output give us? How might we report this information in a report?

> Does total litter biomass differ among sites at the Niwot Ridge LTER? Yes, litter biomass differs among sites at the Niwot Ridge (figure 1; ANOVA, F11, 198 = 4.813, p < 0.0001)
...
summary(Litter.anova.2way) #both funcgroup and nlcd is significant predictor on dry mass
...
litter.anova.newplot <- ggplot(Litter, aes(y = dryMass, x = nlcdClass, color = functionalGroup)) +
  geom_boxplot() +
  labs(x = "Landcover Class", y = "Dry Mass (g)", color = "Functional Group") +
  scale_color_brewer(palette = "Set1") +
  stat_summary(geom = "text", fun.y = max, vjust = -1, hjust = 1, size = 3.5,
               label = c("a", "b", "bc", "cd", "d","d","d","d","d","d","d","d","d","d","d","d","d","d","d","d","d","d","d","d")) +
  ylim(0, 60)
print(litter.anova.newplot)
