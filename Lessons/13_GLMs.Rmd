...
#lake follows the same slope??, the model knows that lakename is categorical data

# interaction effects
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction)
#intercept is paul lake at depth of 0, if the lake is paul lake for each increase in depth there is 90 

TNplot <- ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = tn_ug, color = lakename)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Depth (m)", y = expression(paste("Total Nitrogen (", mu, "g/L)")), color = "") +
...
TempTest.mixed <- lme(data = NTL.summertemp,
                     temperature_C ~ year4, 
                     random = ~1|lakename)
#random variable is 1, grouped by lakename, random effects need to be categorical 
summary(TempTest.mixed)
#not testing random effect so no p-value 
rsquared(TempTest.mixed)
#marginal r could explain about 7% with out random effect, conditional r could explain 11% of our data with random effect of lakename in our model 
...
#mixed effects model is the better model 

#intercept and slope is from the temptest.mixed model 
NTL.tempmodel <-
...
> ANSWER: surface waters have warmed 1.8 degrees over 30 years (slope 0.6 * 30 years) 
