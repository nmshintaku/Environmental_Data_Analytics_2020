...
```{r, message = FALSE} 
# Load package, put message = F so it doesn't inclue the message when you are knitting
library(tidyverse)
```
...
> ANSWER: This file can be found in the data file folder under metadata. The information in this file is important for reproducible data because it provides the reader/user with information, such as where data was collected, when it was collected, and how it was collected. It better informs the user on exactly what the csv file contains and the meaning behind the data. 
...
> Answer: Numeric became NA and factor stayed blank 
...
> Answer: presence vs absence data you could remove NAs
corralation matrix --> remove NAs 
...
```{r}
#Origin Date in R is Jan 1, 1970
help(as.Date)
