...
# Fix this code, put the color code outside of the aes func
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2)

# Add additional variables, as.factor because you want year to be categorical instead of an integer
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) +
  geom_point()
print(PMplot)

# Separate plot with facets, facet wrap needs a vars 
PMplot.faceted <-
...
# can also code Site.Name %in% c("Clemmons Middle", "Leggett", "Bryson City")
#facet grid is sensitive to order you put the variables; site.name is on row and year is column
...
print(PeterPaultemp2)

#Kateri's code 
PP.temp <- ggplot(subset(PeterPaul.chem.nutrients, depth == 0)) +
...
#kateri's code
PP.temp2 <- ggplot(PeterPaul.chem.nutrients) +
...
AQIgraph <- ggplot(EPAair) +
  geom_point(aes(x = PM2.5, y = Ozone, color = SITE_LATITUDE), alpha = 0.5) +
