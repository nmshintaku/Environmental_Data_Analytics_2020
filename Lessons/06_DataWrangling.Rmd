...
> Add notes here: You can track changes in ecosystem health, human interactions with the lake, and trend changes in each of these measurements. Tracking trends can influence policy and adaptation strategies. 
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #all rows where depth = 0 with all the columns 

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0)
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25)

# Did the methods arrive at the same result? Yes, it does 
...
> Answer: Rows could change if you dataset changes. Use filter to meet certain conditions or attributes/
...
#East Long and Hummingbird Lake have the highest temperatures with the day number around 182-200 (summer) and depths of zero and 0.5 (shallow)
...
#select columns lakename and sampledate through temperature_C columns
# : means through
#either all include or all exclude; to exlcude use -
```
### Mutate
...
#design a pipe by coding the df followed by %>%
```
