...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) #gets rid of the other objects we don't want in the df
summary(NTL.nutrient.data.PeterPaul$lakename)
...
#How to Pipe
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #dataset name
  mutate(month = month(sampledate)) %>% #add a month column
  select(lakeid:daynum, month, sampledate:comments) %>% #reorder the columns with new month column 
  drop_na(depth) #dropping all depth rows that have NA
...
#remove NAs in the concentration column
...
```{r}
#inputting all the litter files with a name followed by .csv
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE)
LitterFiles

#combine all the month files into one large file 
Litter <- LitterFiles %>%
  ldply(read.csv)
```
