...
EPAair.subsample <- sample_n(EPAair, 5000) #random 5000 samples bc shapiro can only do 5000 samples

# Evaluate assumption of normal distribution
shapiro.test((EPAair.subsample$Ozone)) #pvalue is small so our data is not normally distributed
...
O3.onesample
#mean of 50, alternative hypo is mean < 50, p-value = 2.2e-16 significance 
...
> We can reject the null hypothesis the the ozone level mean is equal to 50 so we can conclude that the ozone level are below the thresholds of a "good" AQI index with the mean value being below 50. (t = -57.98, df = 6829, P-value = 2.2e-16)

> EPA ozone measurements for 2018-2019 were significantly lower than 50, the AQI threshold for "good" air quality (one sample t-test; t = -57.98, df = 6829, P-value < 2.2e-16 )
...
```{r}
shapiro.test(EPAair$Ozone[EPAair$Year == 2018]) #not normally distributed
shapiro.test(EPAair$Ozone[EPAair$Year == 2019]) #not normally distributed
var.test(EPAair$Ozone ~ EPAair$Year) #variances are significantly different
...
O3.twosample #reject null that mean are equal, the means are significantly different 
...
ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) +
  ...
  labs(x = "Ozone Levels", y = "Density", fill = "Year") +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0))

#2
ggplot(EPAair, aes(x = as.factor(Year), y = Ozone, color = as.factor(Year))) +
  geom_boxplot() +
  labs(x = "Year", y = "Ozone", color = "Year") +
  stat_summary(aes(x = as.factor(Year)))
